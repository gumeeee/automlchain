...
class LocalProvider(BaseProvider):
    ...
    def _create_training_script(
        ...
        if use_qlora:
            # QLoRA setup - must be OUTSIDE torch.cuda.is_available() check
            # otherwise config is never added and model loads in fp16
            script_lines.extend([
                "    # QLoRA Setup (always runs if use_qlora=True)",
                ...
                '    print("QLoRA config created")',
                "",
                "    # Verify CUDA is available (required for quantization)",
                "    if not torch.cuda.is_available():",
                '        raise RuntimeError("CRITICAL: QLoRA requires CUDA but GPU not available!")',
        ...
        script_lines.extend([
            ...
            "",
            "    # Check model memory - should be ~1-2GB with QLoRA, ~14GB without",
            "    if torch.cuda.is_available():",
            "        mem_allocated = torch.cuda.memory_allocated() / 1e9",
            "        mem_reserved = torch.cuda.memory_reserved() / 1e9",
            '        print(f"GPU memory: {mem_allocated:.2f}GB allocated, {mem_reserved:.2f}GB reserved")',
            "        if mem_allocated > 5:",
            '            print("WARNING: High memory usage! Model may not be quantized.")',
            "",
            "    # Setup LoRA",
